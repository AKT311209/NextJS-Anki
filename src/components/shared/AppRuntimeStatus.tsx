    ...
    return (
        <div className="sticky top-0 z-50 border-b border-slate-800/70 bg-slate-950/80 backdrop-blur">
            <div className="mx-auto flex w-full max-w-6xl flex-wrap items-center justify-between gap-2 px-4 py-2 text-xs sm:px-6">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    {mounted && <span className="text-slate-400">{backupStatusLabel}</span>}
                    {mounted && backupReminderVisible ? (
                        <>
                            <Link
                                href="/settings#backup"
                                className="rounded-md border border-indigo-700/70 bg-indigo-500/15 px-2 py-1 text-[11px] font-medium text-indigo-200 transition hover:bg-indigo-500/25"
                            >
                                Backup now
                            </Link>
                            <button
